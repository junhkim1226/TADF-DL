...
parser.add_argument("--hidden_dim", help="dimension of GCN layer", type=int, default=256)
parser.add_argument("--N_GCN_layer", help="Number of GCN layer", type=int, default=3)
...
if __name__ == "__main__":
    ...
    train_dataset, train_dataloader = get_dataset_dataloader(
        train_file, batch_size=args.batch_size, shuffle=True)
    val_dataset, val_dataloader = get_dataset_dataloader(
        val_file, batch_size=args.batch_size, shuffle=True)
    ...
    for epoch in range(args.num_epochs):
        ...
        for i_batch, batch in enumerate(train_dataloader):
            ...
            x = train_feature_dict['af']
            adj = train_feature_dict['adj']
            mask = train_feature_dict['atom_mask']

            predict = model(x, adj, mask)
            HOMO, LUMO = train_feature_dict["HOMO"], train_feature_dict["LUMO"]
            S1, T1 = train_feature_dict["S1"], train_feature_dict["T1"]
        ...
        for i_batch, batch in enumerate(val_dataloader):
            ...
            x = val_feature_dict['af']
            adj = val_feature_dict['adj']
            mask = val_feature_dict['atom_mask']

            predict = model(x, adj, mask)
            HOMO, LUMO = val_feature_dict["HOMO"], val_feature_dict["LUMO"]
            S1, T1 = val_feature_dict["S1"], val_feature_dict["T1"]
            property = torch.tensor(list(zip(HOMO,LUMO,S1,T1))).to(device)
