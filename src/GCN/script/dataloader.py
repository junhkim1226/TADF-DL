...
if __name__ == "__main__":
    ...
    for idx,batch in enumerate(data_loader):
        print(batch['af'].shape)
        break

'''\
    smiles = "C1CCCCC1"
    ret = mol_to_feature(smiles)
    feature_adj = ret["fadj"]
    a = torch.from_numpy(feature_adj)
    print(a)
    ...
    for i in range(b.shape[-1]):
        print(b[:,:,i])
    exit(-1)
    fn = "/home/wykgroup/mseok/data/GCN/logp/total.txt"
    dataset, data_loader = get_dataset_dataloader(
        fn, batch_size=32, num_workers=4)
    data_iter = iter(data_loader)
    sample = next(data_iter)
    adj = sample["adj"]
    # print(bf.shape)
