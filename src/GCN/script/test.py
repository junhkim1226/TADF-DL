...
parser.add_argument("--hidden_dim", help="dimension of GCNr layer", type=int, default=4096)
...
if __name__ == "__main__":
    ...
    test_dataset, test_dataloader = get_dataset_dataloader(
        test_file, batch_size=args.batch_size, shuffle=False)
    ...
    with torch.no_grad():
        ...
        for i_batch, batch in enumerate(test_dataloader):
            ...
            x = test_feature_dict['af']
            adj = test_feature_dict['adj']
            mask = test_feature_dict['atom_mask']

            predict = model(x, adj, mask)
            HOMO, LUMO = test_feature_dict["HOMO"], test_feature_dict["LUMO"]
            S1, T1 = test_feature_dict["S1"], test_feature_dict["T1"]
